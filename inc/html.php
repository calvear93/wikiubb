...
function html_login(){
    ...
    print '<div class="centeralign animated bounceIn">'.NL;
...
function html_index($ns){
    ...
    echo '<div id="index__tree" class="animated bounceInRight">';
...
function html_msgarea(){
    ...
    foreach($MSG as $msg){
        ...
        if(info_msg_allowed($msg)) {
          ...
          print '<div class="alert alert-'.$level.' '.$animation.'"><a href="#" class="close" data-dismiss="alert">&times;</a>';
...
function html_register(){
    ...
    // loads and shows the university degrees.
    $file = io_readFile('conf/degrees.list',false);
    $degrees = explode("\n", $file);

    $names = array();
    foreach ($degrees as $career) {
    	array_push( $names, trim(explode(":", $career)[1]) );
    }
    // attribs for selectpicker.
    $select_attrs = array(
                          'data-live-search'=>'true',
                          'data-width'=>'192',
                          'data-size'=>'8',
						  'data-header'=>'Seleccione su carrera',
						  'mobile'=>'true'
                          );
    array_unshift($names, $lang['none_career']);
    // creates a list chooser from degrees.
    $form->addElement(form_makeMenuField('degree', $names, '', $lang['degree'], '', 'selectpicker show-tick', $select_attrs));
