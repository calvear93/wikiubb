...
function register() {
    ...
    $degree  = $INPUT->post->str('degree');
    $groups   = $INPUT->post->arr('groups');
    ...
    if(($domain == $lang['student_domain']) && ($degree == $lang['none_career'])) {
    ...
    // loads degree list.
      ...
      //verifies if user is teacher or other.
      ...
      foreach ($array as $degree_aux)
        if($degree == trim($degree_aux[1]))
          $post['groups'][] = trim($degree[0]);

    //okay try to create the user
    if(!$auth->triggerUserMod('create', array($login, $pass, $fullname, $email, $groups))) {
        msg($lang['regfail'], -1);
        return false;
    }
