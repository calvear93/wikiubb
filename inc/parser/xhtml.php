...
class Doku_Renderer_xhtml extends Doku_Renderer {
    ...
    function rss($url, $params) {
		...
		if(filter_var($url, FILTER_VALIDATE_URL) === false) {
			$msg = '¡URL "'.$url.'" inválida!';
		}

		if(strpos(@file_get_contents($url),'<?xml') === false) {
			$msg = '¡Feed "'.$url.'" inválido!';
		}
        ...
        $feed = new FeedParser(true);
        $feed->set_feed_url($url);
        $feed->set_item_limit($params['max']);
        ...
        if($rc) {
        	$this->doc .= '<h3><span class="fa fa-rss faa-pulse animated faa-slow"></span>&nbsp&nbsp'.$feed->get_title().'</h3>';
	        $this->doc .= '<p><i>'.$feed->get_description();

	        $this->doc .= '<ul class="rss">';
            for($x = $start; $x != $end; $x += $mod) {
                $item = $feed->get_item($x);
                $this->doc .= '<li class="list-unstyled"><div class="li faa-horizontal animated-hover faa-slow">';
                ...
                if($params['date']) {
                	...
                	$this->doc .= '<sub>&nbsp&nbsp<span class="glyphicon glyphicon-pencil"></span>';
                    $this->doc .= ' publicado el <strong>'.$item->get_date("Y/m/d").'</strong>';
            		$this->doc .=' a las '.$item->get_date("H:i").'</sub>';
                }
                if($params['author']) {
                    $author = $item->get_author();
                    if($author) {
                    	$this->doc .= '<sub>&nbsp&nbsp<span class="glyphicon glyphicon-user"></span>';
                        $name = $author->get_name();
                        if(!$name) $name = $author->get_email();
                        if($name) $this->doc .= ' '.$lang['by'].' '.$name.'</sub>';
                    }
                }
                if($params['details']) {
            ...
            $this->doc .= '<div class="li"><span class="glyphicon glyphicon-warning-sign faa-flash animated faa-slow"></span>&nbsp&nbsp';
