    ...
    function init(&$event, $param) {
        ...
        if ($this->multiNsOk(true)) {
            ...
            if (substr($this->getConf('templates_path'),0,10) == 'data/pages') {
                ...
                msg("Opción de Userhomepage [<code>templates_path</code>] debe cambiarse a una ruta relativa a la carpeta de datos (según lo establecido por los ajustes [<code>savedir</code>]). El valor actual se basa en el valor predeterminado (ej. <code>data/pages/...</code>) este mensaje continuará apareciendo hasta que se corrija el valor, compruebe <a href='https://www.dokuwiki.org/plugin:userhomepage'>esta página</a> para más detalles.",2);
            } else {
        ...
        if ($this->multiNsOk()) {
            ...
            if (($this->userOk())&&($_REQUEST['id']!=$this->private_page)) {
                ...
                if ($this->getConf('create_private_ns') && is_file($this->dataDir.'/'.$this->getConf('templates_path').'/userhomepage_private.txt') && !page_exists($this->private_page) && !checklock($this->private_page) && !checkwordblock() && ($this->userOk('private'))) {
                    ...
                    saveWikiText($this->private_page,$this->applyTemplate('private'),'Creada automáticamente');
                    ...
                    foreach($objects as $objectName => $object){
                        ...
                        if ((is_file($this->dataDir.'/'.$this->getConf('templates_path').'/uhp_private_skeleton'.$file)) and (strpos($file, '.txt') !== false)) {
                            ...
                            if (!page_exists($this->custom_target)) {
                                ...
                                msg("Agregada página  '".$this->custom_target."' desde el espacio de nombres de plantillas a tu espacio de nombres privado.",0);
                ...
                if ($this->getConf('create_public_page') && is_file($this->dataDir.'/'.$this->getConf('templates_path').'/userhomepage_public.txt') && !page_exists($this->public_page) && !checklock($this->public_page) && !checkwordblock() && ($this->userOk('public'))) {
                    ...
                    saveWikiText($this->public_page,$this->applyTemplate('public'),'Creada automáticamente');
    ...
    function acl(&$event, $param) {
        ...
        if ($this->multiNsOk()) {
            if ((!$this->getConf('no_acl')) && ($conf['useacl']) && ($this->userOk())) {
                ...
                $i = count($newLines);
                if ($i > 0) {
                    msg("Actualizando ".$i." reglas ACL.",1);
    ...
    function multiNsOk($msg=false) {
        ...
        if (strpos($this->getConf('public_pages_ns'),':%NAME%:%START%') !== false) {
            ...
            if ($PublicNS == $PrivateNS) {
                if ($msg) {
                    msg("Ajustes de Userhomepage en conflicto. Asegúrese de que los espacios de nombres privado y público son diferentes. El módulo no tendrá efecto hasta que esto se corrija.", -1);
                }
                return false;
            } else {
                return true;
