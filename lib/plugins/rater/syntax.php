...
/******************************************************************************/
          ...
          if(isset($_REQUEST["rate".$rater_id])){
           if(isset($_REQUEST["rating_".$rater_id])){
                 ...
                 if($rater_ip_voting_restriction){
                    ...
                    foreach($rater_data as $d){
                  	...
                  	if($rater_ip_vote_count > ($rater_ip_vote_qty - 1)){
                       $rater_msg="<div class='alert alert-danger animated flipInX'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>".$rater_already_rated_msg."</div>";
                  	}else{
                       fwrite($rater_file,$rater_rating."|".$rater_ip.$rater_end_of_line_char);
                       $rater_msg="<div class='alert alert-success animated flipInX'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>".$rater_thankyou_msg."</div>";
                  	}
                 }else{
                   fwrite($rater_file,$rater_rating."|".$rater_ip.$rater_end_of_line_char);
                   $rater_msg="<div class='alert alert-success animated flipInX'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>".$rater_thankyou_msg."</div>";                 }
              ...
              $rater_msg="<div class='alert alert-warning animated flipInX'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>".$rater_not_selected_msg."</div>";
           }
          }
...
/******************************************************************************/
          if ($rater_type=="rate") {
              ...
              {  $ret_details = '<div class="rating__details">'.sprintf($msg_ratingended,date('d.m.Y',strtotime($data['rater_end']))).'<br />';
                 $alink_Details = '<a href="#'.$anker_id.'" onclick="hidden'.$rater_id.'()">(Detalles)</a>'; }
              ...
              if ($data['rater_tracedetails']==='1') {
                  ...
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/1star.png?w=40&amp;" alt="1 Star" width="40" align="left" /> '.$r1.' visitor votes<br />';
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/2star.png?w=40&amp;" alt="2 Stars" width="40" align="left" /> '.$r2.' visitor votes<br />';
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/3star.png?w=40&amp;" alt="3 Stars" width="40" align="left" /> '.$r3.' visitor votes<br />';
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/4star.png?w=40&amp;" alt="4 Stars" width="40" align="left" /> '.$r4.' visitor votes<br />';
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/5star.png?w=40&amp;" alt="5 Stars" width="40" align="left" /> '.$r5.' visitor votes';
              }
              $ret_details .= '</p>';
              ...
              if($rater_img!=false) {
                ...
                $rater_img_xhtml = str_replace("div class='alert alert-success'","",$rater_img_xhtml);
                $rater_img_xhtml = str_replace("</div>","",$rater_img_xhtml);
              ...
              $ret .= '<tr>
                        ...
                        <img src="'.$rater_stars.'?x='.uniqid((double)microtime()*1000000,1).'" alt="'.$rater_stars_txt.' stars" />&nbsp;'.$rater_stars_txt.' con
                        <span class="reviewcount"> '.$rater_votes.' votos '.$alink_Details.'</span><br />';
              ...
              if (($rater_end!='never') && (strtotime($today) > strtotime($rater_end)))
              {
                  ...
                  $ret .= '<button type="submit" class="btn btn-success btn-xs" name="rate'.$rater_id.'">'.$btn_submit.'</button>';
...
/******************************************************************************/
          elseif ($rater_type=="vote") {
              ...
              if ($data['rater_tracedetails']==='1') {
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/like.png?h=12&amp;" alt="Pro" align="left" /> <p align="left">('.$vote1.') </p><p>'. $who1.'</p><br />';
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/dislike.png?h=12&amp;" alt="Contra" align="left" /> <p align="left">('.$vote2.') </p><p>'. $who2.'</p>';
              }
              $ret_details .= '</p>';
...
//              $ret .= '<form name="'.$data['rater_id'].$data['rater_name'].$data['rater_type'].'" method="post" action="doku.php?id=' . $ID .'">';
              ...
              if($data['rater_tracedetails']==='1') {
                    $ret .= '<a href="#'.$anker_id.'" onclick="hidden'.$rater_id.'()">(Detalles)</a><br />'.
          ...
          elseif (($rater_type=="stat") || ($rater_type=="localstat")) {
            ...
            for ($a=0;$a<count($found_votings);$a++) {
                ...
                if(is_file($rater_filename)) {
                    ...
                    $rater_thumbs = '<div width="100%"><img src="'.DOKU_BASE.'lib/plugins/rater/img/like.png?h=12&amp;" alt="Pro" /><span style="color:#5b8f22">&nbsp;('.$tmp_array[0][1].')&nbsp;</span>'.
                                     '<span>&nbsp;'.number_format($calc_result,2,'.','').'% &nbsp;</span>'.NL;

                    if($data['rater_tracedetails']=== '1') {
                        $rater_thumbs .= $ret_script1.
                                     '<a href="#'.$anker_id.'" onclick="hidden'.$blink_id.'()" style="float: right;">(Detalles)</a><br />'.
                    ...
                    $rater_thumbs .= '<div width="100%"><img src="'.DOKU_BASE.'lib/plugins/rater/img/dislike.png?h=12&amp;" alt="Contra" /><span style="color:#FF1822">&nbsp;('.$tmp_array[0][3].')&nbsp;</span>'.
                                     '<span>&nbsp;'.number_format($calc_result,2,'.','').'% &nbsp;</span>'.NL;

                    if($data['rater_tracedetails']=== '1') {
                        $rater_thumbs .= $ret_script2.
                                         '<a href="#'.$anker_id2.'" onclick="hidden'.$blink_id2.'()" style="float: right;">(Detalles)</a><br />'.
            ...
            $ret .= ' <table class="table sortable rating_stat_table" id="rating_stat_table">
                         <thead>
                             <tr  class="">
                                '.$d2.'
                                <th class="rating_stat_th" style="cursor: pointer;">Item</th>
                                <th class="rating_stat_th" style="cursor: pointer;">Puntaje</th>
                                <th class="rating_stat_th" style="cursor: pointer;">Detalles</th>
                             </tr>
                         </thead>
                         <tbody>';
            ...
            foreach($found_ratings as $findings) {
                ...
                if($data['rater_tracedetails']=== '1') {
                    $ret_details ='<p class="rating__details">';
                    $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/1stars.png?w=40&amp;" alt="1 Star" width="40" align="left" /> '.$findings[2][0].' visitor votes<br />';
                    $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/2stars.png?w=40&amp;" alt="2 Stars" width="40" align="left" /> '.$findings[2][1].' visitor votes<br />';
                    $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/3stars.png?w=40&amp;" alt="3 Stars" width="40" align="left" /> '.$findings[2][2].' visitor votes<br />';
                    $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/4stars.png?w=40&amp;" alt="4 Stars" width="40" align="left" /> '.$findings[2][3].' visitor votes<br />';
                    $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/5stars.png?w=40&amp;" alt="5 Stars" width="40" align="left" /> '.$findings[2][4].' visitor votes';
                    $ret_details .= '</p>';
                    $ret_script1  = str_ireplace('$blink_id', $blink_id, $ret_script);
            ...
            foreach($found_votings as $findings) {
              ...
              if($data['rater_tracedetails']=== '1') {
                  // build the return value for details
                  $ret_details = '<p class="rating__details">';
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/like.png?h=12&amp;" alt="Pro" align="left" /> <p align="left">('.$findings[0][0].') </p><p>'. $findings[0][1].'</p><br />';
                  $ret_details .= '<img src="'.DOKU_BASE.'lib/plugins/rater/img/dislike.png?h=12&amp;" alt="Contra" align="left" /> <p align="left">('.$findings[0][2].') </p><p>'. $findings[0][3].'</p>';
                  $ret_details .= '</p>';
              }
...
/******************************************************************************/
  function assign_star_image($rater_rating) {
                  // Assign star image
              if ($rater_rating <= 0  ){$rater_stars = DOKU_BASE."lib/plugins/rater/img/00stars.png";$rater_stars_txt="Sin valoración";}
              if ($rater_rating >= 0.5){$rater_stars = DOKU_BASE."lib/plugins/rater/img/05stars.png";$rater_stars_txt="0.5";}
              if ($rater_rating >= 1  ){$rater_stars = DOKU_BASE."lib/plugins/rater/img/1stars.png";$rater_stars_txt="1";}
              if ($rater_rating >= 1.5){$rater_stars = DOKU_BASE."lib/plugins/rater/img/15stars.png";$rater_stars_txt="1.5";}
              if ($rater_rating >= 2  ){$rater_stars = DOKU_BASE."lib/plugins/rater/img/2stars.png";$rater_stars_txt="2";}
              if ($rater_rating >= 2.5){$rater_stars = DOKU_BASE."lib/plugins/rater/img/25stars.png";$rater_stars_txt="2.5";}
              if ($rater_rating >= 3  ){$rater_stars = DOKU_BASE."lib/plugins/rater/img/3stars.png";$rater_stars_txt="3";}
              if ($rater_rating >= 3.5){$rater_stars = DOKU_BASE."lib/plugins/rater/img/35stars.png";$rater_stars_txt="3.5";}
              if ($rater_rating >= 4  ){$rater_stars = DOKU_BASE."lib/plugins/rater/img/4stars.png";$rater_stars_txt="4";}
              if ($rater_rating >= 4.5){$rater_stars = DOKU_BASE."lib/plugins/rater/img/45stars.png";$rater_stars_txt="4.5";}
              if ($rater_rating >= 5  ){$rater_stars = DOKU_BASE."lib/plugins/rater/img/5stars.png";$rater_stars_txt="5";}
              return $rater_stars;
  }
/******************************************************************************/
