...
//          msg("eol_char = |".$rater_end_of_line_char."|",0);
          ...
          if (($rater_end!='never') && (strtotime($today) > strtotime($rater_end)))
              {$rater_endmsg =sprintf($msg_votended,date('d.m.Y',strtotime($rater_end))).'<br>';
               ...
               //echo '<meta http-equiv="refresh" content="1; URL=doku.php?id='.$ID.'#'.$anker_id.'"><div class="thumb__negative_feedback">'.$rater_endmsg.'<a href="doku.php?id='.$ID.'" />'.$alink_Back.'</a></div>';
               echo '<div class="alert alert-success animated flipInX">IP'.$rater_endmsg.'</div>';
               return;
               }
                	...
                	if($rater_ip_vote_count > ($rater_ip_vote_qty - 1)){
                     ...
                     if($rater_rating===2) {
                        $rater_msg .= $this->getLang('msg_why');
                         $addMXG = '';
                         //echo '<div class="thumb__positive_feedback">'.$rater_ip.' : '.$rater_msg.'<br />'.'<a href="doku.php?id='.$ID.'#'.$anker_id.'" />'.$alink_Back.'</a></div>';
                         echo '<div class="alert alert-info animated flipInX">'.$rater_msg.'</div>';
                         return;
                      }
                	}
               }
