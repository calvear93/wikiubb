...
class helper_plugin_pagelist extends DokuWiki_Plugin {
    ...
    function _diffCell($id) {
        ...
        $url_params = array();
        $url_params ['do'] = 'diff';
        //$content = '<a href="'.wl($id, $url_params).($this->page['section'] ? '#'.$this->page['section'] : '').'" class="diff_link"><img src="/lib/images/diff.png" width="15" height="11" title="'.hsc($this->getLang('diff_title')).'" alt="'.hsc($this->getLang('diff_alt')).'"/></a>';
        $content = '<a href="'.wl($id, $url_params).($this->page['section'] ? '#'.$this->page['section'] : '').'" class="btn btn-warning btn-xs" data-toggle="tooltip" title="'.hsc($this->getLang('diff_title')).'"><span class="glyphicon glyphicon-eye-open"></span></a>';
        return $this->_printCell('page', $content);
    }
