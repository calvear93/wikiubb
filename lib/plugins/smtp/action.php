...
class action_plugin_smtp extends DokuWiki_Action_Plugin {
    ...
    public function handle_mail_message_send(Doku_Event &$event, $param) {
        ...
        try {
            ...
            msg('Hubo un problema inesperado comunicándose con el servidor SMTP: '.$e->getMessage(), -1);
            $ok = false;
        }
        ...
        if(!$ok && $this->getConf('debug')) {
            ...
            $log = trim(join("\n", $log));
            msg('SMTP log:<br /><pre>'.hsc($log).'</pre><b>La información puede contener información sensible, como contraseñas. ¡No lo publique!</b>',-1);
        }
