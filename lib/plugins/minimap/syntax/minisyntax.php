        ...
        if ($mode == 'xhtml') {
            ...
            switch ($state) {
                ...
                case DOKU_LEXER_SPECIAL :
                    ...
                    if($parameters['title'])
                        $panelHeaderContent = $parameters['title'];

                    $miniMapPanel .= '<div class="panel-heading">' . $panelHeaderContent . '  <span class="label label-primary">' . $pagesCount . ' página(s)</span></div>';
                    if ($parameters['debug']) {
                        $miniMapPanel .= '<div class="panel-body">' .
                            '<B>Información de Depuración:</B><BR>' .
                            'CallingId: (' . $callingId . ')<BR>' .
                            'Opción Suprimida: (' . $parameters['suppress'] . ')<BR>' .
                            '</div>';
                    }
