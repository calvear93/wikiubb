...
class admin_plugin_groupadmin extends DokuWiki_Admin_Plugin {
    ...
    public function html() {
        ...
        foreach ($degrees as $career) {
            ...
            $aux = explode(":", $career);
            $array[$aux[0]] = trim($aux[1]);
        }

        ptln('  <label for="groupname">'.$this->getLang('selectgroup').'</label>');
        ptln('  <select class="selectpicker show-tick show-menu-arrow"
             data-width="auto" data-size="12" data-live-search="true"
        ...
        foreach ($group_list as $group) {
            ...
            if($array[$group])
                ptln('<option data-content="<span class=\'label label-primary\'>'.$group.'</span>&nbsp'.$array[$group].
                     '" '.$selected.'>'.$group.'</option>');
            else
                ptln('<option data-content="<span class=\'label label-info\'>@'.$group.'</span>&nbsp'.ucfirst($group).'" '.$selected.'>'.$group.'</option>');
            //ptln('<option value="'.$degree[1].'" selected="selected">'.$degree[1].'</option>');
        }
        ...
        if ($this->_group_name) {
            ...
            ptln('<table><tr><td>');
            ptln('    <select id="allusers" class="groupadminselection" multiple="multiple" size="20">');
            foreach ($all_users as $user => $userinfo) {
                extract($userinfo);
            ...
            ptln('    </select>');
            ptln('</td><td>');
            ptln('    <button type="button" class="groupadminselection btn-info" onclick="addSelected()"><span class="glyphicon glyphicon-arrow-right"></span> '.$this->getLang('btn_add').'</button><br/>');
            ptln('    <button type="button" class="groupadminselection btn-info" onclick="addall()"><span class="glyphicon glyphicon-arrow-right"></span> '.$this->getLang('btn_addall').'</button><br/><br/>');
            ptln('    <button type="button" class="groupadminselection btn-danger" onclick="removeSelected()"><span class="glyphicon glyphicon-arrow-left"></span> '.$this->getLang('btn_remove').'</button><br/>');
            ptln('    <button type="button" class="groupadminselection btn-danger" onclick="removeall()"><span class="glyphicon glyphicon-arrow-left"></span> '.$this->getLang('btn_removeall').'</button>');
            ptln('</td><td>');
            ptln('    <select id="groupusers" class="groupadminselection" multiple="multiple" size="20">');
            ...
            ptln('  <br/><input type="submit" name="cmd[save]" class="btn-success" value="'.$this->getLang('btn_save').'" />');
            ptln('  <div>');
        }
        ptln('</form>');
    }
